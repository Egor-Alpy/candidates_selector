...
from app.schemas.request import ESCandidatesRequest, ESCandidatesShrinkedEnumsRequest
...
@router.post('/es_1_shrink_enums', response_model=ESCandidatesResponse)
async def process_collection(
        request: ESCandidatesShrinkedEnumsRequest,
        es_service: ElasticSearchSelector = Depends(get_service_es_selector),
):
    """Получение кандидатов из коллекции индекса ES"""
    try:
...
@router.post('/es_2', response_model=ESCandidatesResponse)
async def process_collection(
        request: ESCandidatesRequest,
        es_service: ElasticSearchSelector = Depends(get_service_es_selector),
):
    """Получение кандидатов из коллекции индекса ES"""
    try:
        body = ElasticQueries.get_query_v2(
            position_title=request.position_title,
            yandex_category=request.position_yandex_category,
            size=request.size
        )
